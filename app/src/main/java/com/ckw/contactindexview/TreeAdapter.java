...
public class TreeAdapter extends BaseAdapter {
    ...
    //判断当前Id的tempPoint是否展开了,只要判断它的父节点是否展开就可以了
    ...
    /*
    * 这里展示的是显示着的数据，
    * 根据输入的position，获得实际展示的第position条数据
    * */
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if ("0".equals(tempPoint.getISLEAF())) {  //如果为父节点
            if (!tempPoint.isExpand()) {    //不展开显示加号
                ...
                holder.icon.setImageResource(R.mipmap.outline_list_collapse);
            } else {                        //展开显示减号
                holder.icon.setVisibility(View.VISIBLE);
                holder.icon.setImageResource(R.mipmap.outline_list_expand);
            }
        } else {   //如果叶子节点，不占位显示
